    ...
    <div className="p-6 space-y-6 bg-slate-950 min-h-screen text-slate-100">
      <div className="flex items-center justify-between bg-slate-900 p-6 rounded-2xl border border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Accounts Payable (AP) Vendor Aging Analysis</h1>
            <p className="text-xs text-slate-400 mt-0.5">Supplier Outstanding Matrix • 0-30, 31-60, 61-90, 90+ Days Payment Buckets</p>
          </div>
        </div>

        <div className="text-right">
          <div className="text-xs text-slate-400">Total Vendor Payables</div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        <table className="w-full text-left text-xs text-slate-300">
          ...
          <tbody className="divide-y divide-slate-800/60 font-mono">
            {filtered.map((p) => (
              <tr key={p.supplierId} className="hover:bg-slate-800/40 transition">
                <td className="py-3.5 px-4 font-sans font-bold text-white">{p.supplierName}</td>
                <td className="py-3.5 px-4 text-right text-emerald-400">₹{(p.days0to30 || p.days1_30 || p.current || 0).toLocaleString()}</td>
                <td className="py-3.5 px-4 text-right text-blue-400">₹{(p.days31to60 || p.days31_60 || 0).toLocaleString()}</td>
                ...
                <td className="py-3.5 px-4 text-right font-bold text-white">₹{p.totalOutstanding.toLocaleString()}</td>
                <td className="py-3.5 px-4 text-center font-sans">
                  <Link
                    href="/accounting/payments"
                    className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-semibold rounded-lg transition inline-flex items-center gap-1"
                  >
